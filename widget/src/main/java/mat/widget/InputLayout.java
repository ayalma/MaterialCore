...
public class InputLayout extends RelativeLayout {
    ...
    public InputLayout(Context context) {
        super(context);
        initInputLayout(null, R.attr.material_inputLayoutStyle);
    }

    public InputLayout(Context context, AttributeSet attrs) {
        super(Material.getThemedContext(context, attrs, R.styleable.InputLayout, R.attr.material_inputLayoutStyle, R.styleable.InputLayout_material_theme), attrs);
        initInputLayout(attrs, R.attr.material_inputLayoutStyle);
    }

    public InputLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(Material.getThemedContext(context, attrs, R.styleable.InputLayout, defStyleAttr, R.styleable.InputLayout_material_theme), attrs, defStyleAttr);
        initInputLayout(attrs, defStyleAttr);
    }

    public InputLayout(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(Material.getThemedContext(context, attrs, R.styleable.InputLayout, defStyleAttr, R.styleable.InputLayout_material_theme), attrs, defStyleAttr, defStyleRes);
        initInputLayout(attrs, defStyleAttr);
    }

    private void initInputLayout(AttributeSet attrs, int defStyleAttr) {
        View.inflate(getContext(), R.layout.material_inputlayout, this);

        errorTextView = findViewById(R.id.material_error);
        errorTextView.setTextColor(new DefaultAccentColorStateList(getContext()));
        errorTextView.setValid(false);
        counterTextView = findViewById(R.id.material_counter);
        labelTextView = findViewById(R.id.material_label);
        labelTextView.setTextColor(new DefaultAccentColorStateList(getContext()));
        labelTextView.setGravity(gravity);
        clearImageView = findViewById(R.id.material_clear);
        showPasswordImageView = findViewById(R.id.material_showPassword);
        voiceInputImageView = findViewById(R.id.material_voiceInput);
        container = findViewById(R.id.material_inputLayoutContainer);
        ...
        for (int i = 0; i < a.getIndexCount(); i++) {
            ...
            if (!isInEditMode() && attr == R.styleable.InputLayout_material_errorFontPath) {
                String path = a.getString(attr);
                Typeface typeface = TypefaceUtils.getTypeface(getContext(), path);
                setErrorTypeface(typeface);
            } else if (attr == R.styleable.InputLayout_material_errorTextSize) {
                setErrorTextSize(a.getDimension(attr, 0));
            } else if (attr == R.styleable.InputLayout_material_errorFontFamily) {
                ...
                setErrorTypeface(typeface);
            } else if (!isInEditMode() && attr == R.styleable.InputLayout_material_labelFontPath) {
                String path = a.getString(attr);
                Typeface typeface = TypefaceUtils.getTypeface(getContext(), path);
                setLabelTypeface(typeface);
            } else if (attr == R.styleable.InputLayout_material_counterTextSize) {
                setCounterTextSize(a.getDimension(attr, 0));
            } else if (attr == R.styleable.InputLayout_material_labelFontFamily) {
                int textStyle = a.getInt(R.styleable.InputLayout_android_textStyle, 0);
                Typeface typeface = TypefaceUtils.getTypeface(getContext(), a.getString(attr), textStyle);
                setLabelTypeface(typeface);
            } else if (!isInEditMode() && attr == R.styleable.InputLayout_material_counterFontPath) {
                String path = a.getString(attr);
                Typeface typeface = TypefaceUtils.getTypeface(getContext(), path);
                setCounterTypeface(typeface);
            } else if (attr == R.styleable.InputLayout_material_labelTextSize) {
                setLabelTextSize(a.getDimension(attr, 0));
            } else if (attr == R.styleable.InputLayout_material_counterFontFamily) {
                int textStyle = a.getInt(R.styleable.InputLayout_android_textStyle, 0);
                Typeface typeface = TypefaceUtils.getTypeface(getContext(), a.getString(attr), textStyle);
                setCounterTypeface(typeface);
            }
        ...
        String error = a.getString(R.styleable.InputLayout_material_error);
        setError(error == null ? a.getString(R.styleable.InputLayout_material_errorMessage) : error);
        setErrorMode(ErrorMode.values()[a.getInt(R.styleable.InputLayout_material_errorMode, ErrorMode.WhenInvalid.ordinal())]);

        setLabelStyle(LabelStyle.values()[a.getInt(R.styleable.InputLayout_material_labelStyle, LabelStyle.Floating.ordinal())]);
        setLabel(a.getString(R.styleable.InputLayout_material_label));
        setActionButton(ActionButton.values()[a.getInt(R.styleable.InputLayout_material_actionButton, 0)]);
    ...
    private ViewGroup.LayoutParams setInputChild(View child, android.widget.RelativeLayout.LayoutParams params) {
        ...
        if (child.getId() == NO_ID)
            child.setId(R.id.material_input);
        params.addRule(BELOW, R.id.material_label);
        ...
        if (actionButton != ActionButton.None) {
            int buttonPadding = getResources().getDimensionPixelSize(R.dimen.material_padding) + getResources().getDimensionPixelSize(R.dimen.material_iconSize);
    ...
    /**
     * Deprecated use {@link material.widget.InputLayout.setErrorMode} instead.
     *
    ...
    public void setActionButton(ActionButton actionButton) {
        ...
        if (child != null) {
            ...
            if (this.actionButton != ActionButton.None)
                paddingRight -= getResources().getDimensionPixelSize(R.dimen.material_padding) + getResources().getDimensionPixelSize(R.dimen.material_iconSize);
        }
