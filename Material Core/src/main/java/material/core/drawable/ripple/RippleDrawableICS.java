package material.core.drawable.ripple;
 ...
 * mask using {@code setId(..., R.id.material_mask)} or an existing mask layer
 * may be replaced using {@code setDrawableByLayerId(R.id.material_mask, ...)}.
 * <pre>
...
public class RippleDrawableICS extends LayerDrawable implements RippleDrawable {
    ...
    public RippleDrawableICS(@NonNull ColorStateList color, @Nullable Drawable content,
        ...
        if (mask != null) {
            addLayer(mask, null, R.id.material_mask, 0, 0, 0, 0);
        }
    ...
    public void getOutline(@NonNull Outline outline) {
        ...
        for (int i = 0; i < N; i++) {
            if (children[i].mId != R.id.material_mask) {
    ...
    private void drawContent(Canvas canvas) {
        ...
        final int count = mLayerState.mNum;
        for (int i = 0; i < count; i++) {
            if (array[i].mId != R.id.material_mask) {
    ...
    public Drawable mutate() {
        ...
        mMask = findDrawableByLayerId(R.id.material_mask);
    ...
    @Override
    public int getAlpha() {
        return Material.getDrawableAlpha(background);
    }
}
