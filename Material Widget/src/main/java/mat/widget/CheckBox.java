...
public class CheckBox extends TextView implements Checkable {
    ...
    public CheckBox(Context context, AttributeSet attrs) {
        super(Material.getThemedContext(context, attrs, R.styleable.CheckBox, android.R.attr.checkboxStyle, R.styleable.CheckBox_material_theme), attrs, android.R.attr.checkboxStyle);
        initCheckBox(attrs, android.R.attr.checkboxStyle);
    }

    public CheckBox(Context context, AttributeSet attrs, int defStyleAttr) {
        super(Material.getThemedContext(context, attrs, R.styleable.CheckBox, defStyleAttr, R.styleable.CheckBox_material_theme), attrs, defStyleAttr);
        initCheckBox(attrs, defStyleAttr);
    }

    @TargetApi(Build.VERSION_CODES.LOLLIPOP)
    public CheckBox(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        super(Material.getThemedContext(context, attrs, R.styleable.CheckBox, defStyleAttr, R.styleable.CheckBox_material_theme), attrs, defStyleAttr, defStyleRes);
        initCheckBox(attrs, defStyleAttr);
    }
    ...
    public void initCheckBox(AttributeSet attrs, int defStyleAttr) {
        Drawable d;
        if (!isInEditMode()) {
            d = new CheckableDrawable(getContext(), R.raw.material_checkbox_checked, R.raw.material_checkbox_unchecked, R.raw.material_checkbox_filled, new PointF(-0.09f, 0.11f));
        } else {
            d = getResources().getDrawable(android.R.drawable.checkbox_on_background);
        }
        setButtonDrawable(d);

        TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.CheckBox, defStyleAttr, R.style.material_CheckBox);
        ...
        for (int i = 0; i < a.getIndexCount(); i++) {
            ...
            } else if (attr == R.styleable.CheckBox_material_buttonGravity) {
                buttonGravity = ButtonGravity.values()[a.getInt(attr, 0)];
            }
        }

        Material.initHtmlText(this, a, R.styleable.CheckBox_material_htmlText);
    ...
    private static final int[] INDETERMINATE_STATE_SET = {
            R.attr.material_state_indeterminate
    };
